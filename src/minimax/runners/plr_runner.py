...
class PLRRunner(DRRunner):
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _compile_stats(
        self, update_stats, ep_stats, is_replay, env_metrics=None, plr_stats=None
    ):
        stats = super()._compile_stats(
            update_stats, ep_stats, env_metrics, mask_passable=False
        )
        ...
        if self.track_env_metrics:
            if self.use_parallel_eval:
                ...
                env_env_metrics = {
                    k: v for k, v in env_metrics.items() if k.startswith("env/")
                }
                env_env_metrics = jax.vmap(
                    lambda info, mask: jax.tree.map(
                        lambda x: (x * mask).sum() / mask.sum(), info
                    )
                )(env_env_metrics, env_passable_mask)
                env_env_metrics.update(
                    {
                        "env/passable_ratio": jax.vmap(lambda x: x.mean())(
                            env_passable_mask
                        )
                    }
                )
                ...
                plr_env_metrics = {
                    k: v for k, v in env_metrics.items() if k.startswith("plr/")
                }
                plr_env_metrics = jax.vmap(
                    lambda info, mask: jax.tree.map(
                        lambda x: (x * mask).sum() / mask.sum(), info
                    )
                )(plr_env_metrics, plr_passable_mask)
                plr_env_metrics.update(
                    {
                        "plr/passable_ratio": jax.vmap(lambda x: x.mean())(
                            plr_passable_mask
                        )
                    }
                )
                ...
                env_metrics = jax.vmap(
                    lambda info, mask: jax.tree.map(
                        lambda x: (x * mask).sum() / mask.sum(), info
                    )
                )(env_metrics, passable_mask)
                env_metrics.update(
                    {"passable_ratio": jax.vmap(lambda x: x.mean())(passable_mask)}
                )

            if not self.use_parallel_eval:
                _env_metrics = jax.vmap(
                    lambda cond: jax.lax.cond(
                        cond,
                        lambda *_: {
                            f"env/{k}": -jnp.inf for k, _ in env_metrics.items()
                        },
                        lambda *_: {
                            f"env/{k}": v.mean() for k, v in env_metrics.items()
                        },
                    )
                )(is_replay)
                ...
                plr_env_metrics = jax.vmap(
                    lambda cond: jax.lax.cond(
                        cond,
                        lambda *_: {
                            f"plr/{k}": v.mean() for k, v in env_metrics.items()
                        },
                        lambda *_: {
                            f"plr/{k}": -jnp.inf for k, _ in env_metrics.items()
                        },
                    )
                )(is_replay)
        ...
        if plr_stats is not None:
            plr_stats = jax.vmap(
                lambda info: jax.tree_util.tree_map(lambda x: x.mean(), info)
            )(plr_stats)
    ...
    def run(
        ...
        if self.track_env_metrics:
            if self.use_parallel_eval:
                new_levels = jax.tree.map(
                    lambda x: x.at[:, :n_level_samples].get(), levels
                )
        ...
        stats = self._compile_stats(
            update_stats, ep_stats, is_replay, env_metrics, plr_stats
        )
