...
class DRRunner:
    ...
    def _get_transition(
        ...
        (next_obs, next_state, reward, done, info, extra) = self.benv.step(
            jnp.array(vrngs), state, action, reset_state, extra
        )
    ...
    def _compile_stats(
        self, update_stats, ep_stats, env_metrics=None, mask_passable=True
    ):
        ...
        if self.track_env_metrics:
            if mask_passable:
                ...
                mean_env_metrics.update(
                    {"env/passable_ratio": jax.vmap(lambda x: x.mean())(passable_mask)}
                )
            else:
                mean_env_metrics = jax.vmap(
                    lambda info: jax.tree.map(lambda x: x.mean(), info)
                )(env_metrics)
                mean_env_metrics = {f"env/{k}": v for k, v in mean_env_metrics.items()}
